...
class SetSiteLanguageStep implements StepInterface {
			...
			try {
				...
				copy_between_filesystems(
					array(
						...
						'source_path'       => '/',
						'target_filesystem' => $runtime->get_target_filesystem(),
	...
	private function getWordPressTranslationUrl( Runtime $runtime, string $wp_version, string $language, Client $client ) {
		try {
			...
			$runtime->get_logger()->warning( 'Warning: Failed to fetch translations details from WordPress.org API: ' . $e->getMessage() );
		}
