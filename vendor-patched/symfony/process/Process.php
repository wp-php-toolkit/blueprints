...
class Process implements IteratorAggregate {
	...
	public function __construct( $commandline, $cwd = null, ?array $env = null, $input = null, $timeout = 60, ?array $options = null ) {
		...
		if ( null !== $options ) {
			@trigger_error( sprintf( 'The $options parameter of the %s constructor is deprecated since version 3.3 and will be removed in 4.0.',
				__CLASS__ ), E_USER_DEPRECATED );
	...
	public function start( ?callable $callback = null/*, array $env = array()*/ ) {
		...
		if ( 2 <= func_num_args() ) {
			...
			if ( __CLASS__ !== static::class ) {
				...
				if ( __CLASS__ !== $r->getDeclaringClass()->getName() && ( 2 > $r->getNumberOfParameters() || 'env' !== $r->getParameters()[0]->name ) ) {
					@trigger_error( sprintf( 'The %s::start() method expects a second "$env" argument since version 3.3. It will be made mandatory in 4.0.',
						static::class ), E_USER_DEPRECATED );
				}
			}
			$env = null;
		}
		...
		if ( null !== $env && $inheritEnv ) {
			...
			@trigger_error( 'Not inheriting environment variables is deprecated since Symfony 3.3 and will always happen in 4.0. Set "Process::inheritEnvironmentVariables()" to true instead.',
				E_USER_DEPRECATED );
		}
		...
		$this->process = proc_open(
			is_array(
				is_array( $commandline )
					? implode( ' ', array_map( 'escapeshellarg', $commandline ) )
					: $commandline
			)
				? implode( ' ', array_map(
					'escapeshellarg',
	...
	public function getCommandLine() {
		return is_array( $this->commandline ) ? implode( ' ',
			array_map( array( $this, 'escapeArgument' ), $this->commandline ) ) : $this->commandline;
	}
		...
		if ( $tty ) {
			...
			if ( null === $isTtySupported ) {
				$isTtySupported = (bool) @proc_open( 'echo 1 >/dev/null',
					array( array( 'file', '/dev/tty', 'r' ), array( 'file', '/dev/tty', 'w' ), array( 'file', '/dev/tty', 'w' ) ), $pipes );
			}
	...
	public function getOptions() {
		@trigger_error( sprintf( 'The %s() method is deprecated since version 3.3 and will be removed in 4.0.', __METHOD__ ),
			E_USER_DEPRECATED );
	...
	public function getEnhanceWindowsCompatibility() {
		@trigger_error( sprintf( 'The %s() method is deprecated since version 3.3 and will be removed in 4.0. Enhanced Windows compatibility will always be enabled.',
	...
	public function getEnhanceSigchildCompatibility() {
		@trigger_error( sprintf( 'The %s() method is deprecated since version 3.3 and will be removed in 4.0. Sigchild compatibility will always be enabled.',
			__METHOD__ ), E_USER_DEPRECATED );
	...
	public function areEnvironmentVariablesInherited() {
		@trigger_error( sprintf( 'The %s() method is deprecated since version 3.3 and will be removed in 4.0. Environment variables will always be inherited.',
			__METHOD__ ), E_USER_DEPRECATED );
	...
	private function getDescriptors() {
		...
		if ( '\\' === DIRECTORY_SEPARATOR ) {
			...
			$this->processPipes = new UnixPipes( $this->isTty(), $this->isPty(), $this->input,
				! $this->outputDisabled || $this->hasCallback );
		}
