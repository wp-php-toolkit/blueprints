		...
		foreach ( $container->findTaggedServiceIds( $this->listenerTag, true ) as $id => $events ) {
			...
			foreach ( $events as $event ) {
				...
				if ( ! isset( $event['event'] ) ) {
					throw new InvalidArgumentException( sprintf( 'Service "%s" must define the "event" attribute on "%s" tags.', $id,
						$this->listenerTag ) );
				}
				...
				$definition->addMethodCall( 'addListener',
					array( $event['event'], array( new ServiceClosureArgument( new Reference( $id ) ), $event['method'] ), $priority ) );
			}
		}
