# WordPress Standard Proposal 1: Blueprints version 2
...
can refer to the local Execution Context. This prevents malicious Blueprints
from accessing arbitrary local files.
...
could be a `--may-access-local-files` CLI flag, while for UI runners, it
...
from bundled static files. For security reasons, no URL found in a remote
content file will be considered for rewriting within the Blueprint execution
context.
...
Blueprint runners MAY also support additional semantics.
