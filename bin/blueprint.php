...
$command_configurations = [
	'exec' => [
		...
		'options'         => array_merge( $common_options, [
			 ...
			 *			}
			 */
			'allow'                       => [ null, true, null, 'Allowed permissions. One of: ' . implode( ', ', $supported_permissions ) ],
		] ),
		'examples'        => [
...
function handleExecCommand( array $positional_args, array $options, array $command_config, ProgressReporter $progress_reporter ): void {
	...
	try {
		...
		$config->setProgressObserver( new ProgressObserver( function ( $progress, $caption ) use ( $progress_reporter ) {
...
function cliArgsToRunnerConfiguration( array $positional_args, array $options ): RunnerConfiguration {
	...
	if ( $options['truncate-new-site-directory'] ) {
		...
		} else if( is_dir( $absolute_target_site_root ) ) {
